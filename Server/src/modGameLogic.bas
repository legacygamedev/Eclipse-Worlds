...
Public Sub AddPlayerNewTitle(ByVal index As Long, ByVal TitleNum As Long, Optional ByVal InvNum As Byte, Optional ByVal Message As Boolean = True)
    ...
    If CanAddTitle(index, TitleNum, True) Then
        For i = 1 To MAX_TITLES
            ' Find an empty slot
...
Private Function CanAddTitle(ByVal index As Long, ByVal TitleNum As Byte, Optional ByVal Message As Byte = False) As Boolean
    Dim i As Byte
    
    ' Don't add a title if it has no requirements
    If Title(TitleNum).LevelReq = 0 Or Title(TitleNum).PKReq = 0 Then
